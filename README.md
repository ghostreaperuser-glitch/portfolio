...
/*
 * ╔══════════════════════════════════════════════════════════════════════╗
 *  HOW TO ADD A LECTURER OR STUDENT — READ THIS CAREFULLY
 * ╠══════════════════════════════════════════════════════════════════════╣
 *
 *  STEP 1 — SET UP YOUR PUBLIC DOCUMENTS REPO (do this once)
 *  ──────────────────────────────────────────────────────────
 *  a) Go to github.com → click "New repository"
 *  b) Name it:  uni-contributions
 *  c) Set it to PUBLIC  ← very important
 *  d) Click "Create repository"
 *
 *  STEP 2 — STRUCTURE FOLDERS FOR EACH PERSON
 *  ──────────────────────────────────────────────────────────
 *  Inside that repo, create one folder per contributor.
 *  Each folder can have UP TO 3 FILES:
 *
 *    uni-contributions/
 *      dr-nakato/
 *        coursework.pdf      ← Semester coursework
 *        notes.pdf           ← Lecture notes
 *        README.md           ← Guide telling students what to download
 *      grace-akullo/
 *        notes.pdf
 *        coursework.pdf
 *        README.md
 ...
 *  ──────────────────────────────────────────────────────────
 *  Change GITHUB_USER to your actual GitHub username
 *  Change GITHUB_REPO to the name of your public repo (e.g. uni-contributions)
 *
 *  STEP 4 — ADD THE PERSON TO THE CONTRIBUTORS ARRAY BELOW
 *  ──────────────────────────────────────────────────────────
 *  Copy one of the existing objects and fill it in:
 *
 *  {
 *    id: "unique-id",           ← no spaces, use hyphens (e.g. "dr-nakato")
 *    name: "Dr. Sarah Nakato",  ← full display name
 *    role: "lecturer",          ← "lecturer" or "student"
 *    dept: "cs",                ← filter category: "cs", "eng", or add your own
 *    department: "Computer Science",   ← shown on the card
 *    title: "Senior Lecturer — DSA",   ← shown in the modal
 *    photo: "",                 ← path to their photo e.g. "/assets/contributors/nakato.jpg"
 *                                  OR leave "" for auto-generated initials avatar
 *    semester: "Semester 1, 2025",
 *    githubFolder: "dr-nakato",  ← MUST match the folder name in your GitHub repo exactly
 *    docs: [
 *      { name: "Coursework 1", file: "coursework.pdf", type: "pdf" },
 *      { name: "Lecture Notes", file: "notes.pdf",     type: "pdf" },
 *      { name: "README Guide",  file: "README.md",     type: "md"  },
 *    ]
 *  },
 *
 *  STEP 5 — HOW THE BLUE TICK WORKS
 *  ──────────────────────────────────────────────────────────
 *  Every person in the CONTRIBUTORS array automatically gets a blue tick.
 *  The blue tick is shown by this line in the card HTML:
 *    <i class="fas fa-circle-check" style="color:#0090d4;" title="GhostR Verified">
 *
 *  So your rule is simple:
 *    → If you add someone to this array = they have a blue tick = they are verified
 *    → If you remove someone from this array = they disappear from the page entirely
 *    → You control who gets verified. Only add people you have confirmed personally.
 *
 *  STEP 6 — ADDING A PHOTO FOR A CONTRIBUTOR
 *  ──────────────────────────────────────────────────────────
 *  a) Put their photo (JPG or PNG) in:  /assets/contributors/their-name.jpg
 *     inside your WEBSITE repo (the private one, not the docs repo)
 *  b) Set the photo field to:  "/assets/contributors/their-name.jpg"
 *  c) If you leave photo: "" — the page auto-generates a colored circle with their initials
 *
 *  DOCUMENT FILE TYPES SUPPORTED:
 *  ──────────────────────────────────────────────────────────
 *  type: "pdf"   → red PDF icon
 *  type: "md"    → green README/Markdown icon
 *  type: "txt"   → blue text file icon
 *  type: "other" → yellow generic file icon
 *
 * ╚══════════════════════════════════════════════════════════════════════╝
 */

/* ── CONFIG: change these two values ── */
const GITHUB_USER   = "ghostreaperuser-glitch";   // ← your GitHub username
const GITHUB_REPO   = "uni-contributions";         // ← your public docs repo name
const GITHUB_BRANCH = "main";

const RAW_BASE = `https://raw.githubusercontent.com/${GITHUB_USER}/${GITHUB_REPO}/${GITHUB_BRANCH}`;

/* ── CONTRIBUTORS LIST — add/edit/remove people here ── */
const CONTRIBUTORS = [
  {
    id: "nakato",
    name: "Dr. Sarah Nakato",
...
function docIconClass(type) {
  return { pdf:"fa-file-pdf", md:"fa-file-lines", txt:"fa-file-alt" }[type] || "fa-file";
}
function typeLabel(type) {
  return { pdf:"PDF Document", md:"Markdown / README", txt:"Text File" }[type] || "Document";
}
function buildDownloadUrl(c, filename) {
  return `${RAW_BASE}/${c.githubFolder}/${encodeURIComponent(filename)}`;
}
...
function renderCards(filter) {
  ...
  if (!list.length) {
    grid.innerHTML = `<div class="col-12 empty-state"><i class="fas fa-users-slash"></i><p>No contributors in this category yet.</p></div>`;
    return;
  }

  grid.innerHTML = list.map(c => {
    const avatar = c.photo
      ? `<img src="${c.photo}" class="contrib-avatar" alt="${c.name}"
    ...
    return `
      <div class="col-sm-6 col-md-4 col-lg-3 fade-up">
        <div class="contributor-card" onclick="openDocs('${c.id}')">
          ...
          <div class="contrib-doc-count">
            <i class="fas fa-file-circle-check"></i>
            ${c.docs.length} doc${c.docs.length !== 1 ? 's' : ''} · ${c.semester}
          </div>
        </div>
      </div>`;
  }).join("");
...
function openDocs(id) {
  ...
  if (!c) return;
  document.getElementById("modal-contrib-name").textContent = c.name;
  document.getElementById("modal-contrib-role").textContent = c.title + " · " + c.department;

  const docsHtml = c.docs.map(doc => {
    const url = buildDownloadUrl(c, doc.file);
    return `
      <div class="doc-item">
        <div class="doc-icon ${doc.type || 'other'}"><i class="fas ${docIconClass(doc.type)}"></i></div>
        <div class="doc-meta">
          <div class="doc-name">${doc.name}</div>
          <div class="doc-type-label">${typeLabel(doc.type)} — ${c.semester}</div>
        </div>
        <a href="${url}" class="btn-download" target="_blank" rel="noopener noreferrer" download>
          <i class="fas fa-download me-1"></i> Download
        </a>
  ...
  document.getElementById("modal-body-content").innerHTML = `
    <span class="semester-tag"><i class="fas fa-calendar-alt me-1"></i>${c.semester}</span>
    <p style="font-size:0.85rem;color:var(--muted,#8899aa);margin-bottom:1.2rem;">
      Files are hosted on GitHub. Click Download to open or save each file.
      Always read the README first to understand what's inside.
    </p>
    ${docsHtml}`;

  new bootstrap.Modal(document.getElementById("docs-modal")).show();
}
...
<!--
            ══ HOW TO CONNECT THIS FORM TO YOUR GMAIL ══
            1. Go to https://formspree.io → Sign Up (free)
            2. Create a New Form → set the email to ashrafsoftwareguy@gmail.com
            3. Formspree gives you a form endpoint like: https://formspree.io/f/xyzabcde
            4. Replace YOUR_FORM_ID below with just the ID part (e.g. xyzabcde)
